  ...
  const itemsMarkup = items
    .map(
      (item) => {
        ...
        const titleClass = item.highlighted ? ' class="highlighted-title"' : '';
        return `
        <article class="timeline-item">
    ...
    .map(
      (figure) => `
        <figure class="article-media-card article-figure article-media-layout-${figure.layout}">
          <img src="${figure.url}" loading="lazy" />
...
function renderDetailView(item, collection) {
  ...
  if (Array.isArray(item.content) && item.content.length) {
    // New content array format: render each block in order
    // Group adjacent double-layout media (figure/video) into the same grid row
    let figureCounter = 0;
    let videoCounter = 0;

    const renderMediaCard = (block) => {
      if (block.type === "figure") {
        figureCounter++;
        const url = block.url ? resolveDataUrl(String(block.url)) : "";
        const layout = normalizeMediaLayout(block.layout);
        const caption = block.caption ? String(block.caption) : "";
        const prefix = `Figure ${figureCounter}.`;
        const fullCaption = caption ? `${prefix}  ${caption}` : prefix;
        if (!url) return "";
        return `
          <figure class="article-media-card article-figure article-media-layout-${layout}">
            <img src="${url}" loading="lazy" />
            <figcaption>${fullCaption}</figcaption>
          </figure>
        `;
      }
      if (block.type === "video") {
        ...
        const url = block.url ? resolveDataUrl(String(block.url)) : "";
        const layout = normalizeMediaLayout(block.layout);
        const posterAttr = block.poster ? `poster="${resolveDataUrl(String(block.poster))}"` : "";
        const videoTitle = block.title ? String(block.title) : "";
        const caption = block.caption ? String(block.caption) : "";
        const prefix = `Video ${videoCounter}.`;
        const textParts = [videoTitle, caption].filter(Boolean).join(" - ");
        const fullCaption = textParts ? `${prefix}  ${textParts}` : prefix;
        if (!url) return "";
        return `
          <figure class="article-media-card article-media-video article-media-layout-${layout}">
            <div class="article-video-frame">
              <video class="article-video-player" controls playsinline preload="metadata" ${posterAttr}>
    ...
    const isDoubleMedia = (block) =>
      block && (block.type === "figure" || block.type === "video") && normalizeMediaLayout(block.layout) === "double";
    ...
    while (i < item.content.length) {
      ...
      if (block.type === "body") {
        const text = currentLang === "zh" && block.zhText ? block.zhText : block.text;
        if (text) {
          contentMarkup += `<div class="article-body"><p>${text}</p></div>`;
        }
        i++;
      } else if (block.type === "figure" || block.type === "video") {
        // Collect consecutive double-layout media into one grid
        if (isDoubleMedia(block)) {
          let cards = "";
          while (i < item.content.length && isDoubleMedia(item.content[i])) {
            cards += renderMediaCard(item.content[i]);
            i++;
          }
          contentMarkup += `
            <section class="article-media-gallery" aria-label="Media">
              <div class="article-media-grid">${cards}</div>
            </section>
          `;
        } else {
          // Single layout: render alone
          const card = renderMediaCard(block);
          if (card) {
            contentMarkup += `
              <section class="article-media-gallery" aria-label="Media">
                <div class="article-media-grid">${card}</div>
              </section>
            `;
          }
          i++;
    ...
    // Legacy format: body/videos/figures as separate keys
    const bodyMarkup = `<div class="article-body">${body.map((paragraph) => `<p>${paragraph}</p>`).join("")}</div>`;
    ...
    if (isScenarioDetail) {
      contentMarkup = `${videoGalleryMarkup}${bodyMarkup}`;
    } else {
      contentMarkup = `${figureGalleryMarkup}${bodyMarkup}${videoGalleryMarkup}`;
    }
  }
